...
## [0.9.52] — Check a backup file without restoring it
- New "Check a backup file (changes nothing)" in Pool recovery, directly above Restore. Until now the only way to find out whether a backup was any good was to restore it, because Restore is the only thing that has ever read one — and Restore writes: it saves recipes, tracks covenants and imports coin proofs. So "let me check my file" was a state-changing operation. This is the look-only path.
- It answers the question a user actually has, which "the file exists" does not: can this file still recover my pool, on this node, today. Per pool it reports whether the recipe fields are valid, whether the covenant script rebuilds that exact pool address (the same rule Restore applies, so a pass here means Restore would accept it), how many reserve coins are visible on this node, whether this wallet holds the owner key and how many of its one-time signatures are spent, whether coin proofs are present, and whether the owner key's derivation index was recorded.
- The check that matters most: if the node's signature counter is **below** what the backup recorded, the report says DO NOT RESTORE THIS ONTO THIS NODE and quotes both numbers. That is the shape that precedes a key-reuse accident. A backup that records no count at all is called out too, because nothing can then detect a counter that has gone backwards.
- Where no problem is found, the report says "no problem detected — which is not proof the key is unused". It never claims safety.
- Read-only-ness is enforced by an allowlist in code, not by review: every command is matched on its exact shape, so `coinimport`, `newscript`, `sign`, any `txn*`, bare `keys` (which has creating subcommands) and even `coins address:… megammr:true` are all refused. A future edit that reaches for a writing command fails closed instead of quietly writing during a "check".
- Reports are selectable and copyable with full identifiers, so they paste into a support request.
- 212 tests pass in each build variant (6 new); release lint passes.
...
- A counter regression now quotes both numbers: what this node reports and what the recipe recorded (the 2026-09-09 incident shape was 46 against 590). When several keys are blocked at once, the regression is surfaced first — severity is explicit, not the enum declaration order.
- Results carry the evidence (`nodeUses`, `recordedFloor`, covenant address) so a message needs no second lookup, and every identifier appears in full for copying into a support request. The `Cb` callback's `regenerated` count is gone: it has been hard-zero since owner-key recovery became read-only, and nothing read it.
- Also split the two generic strings behind consolidation and the signature boundary, and the four-way "missing, exhausted, below a recorded count, or the confirmation could not be saved" sentence. Collect still aborts the whole sweep on one blocked key — correct, because core's consolidation picks its own inputs and could otherwise select a coin belonging to that key — and now says which key and why.
- 194 tests pass in each build variant (17 new); release lint passes. No message claims a key is safe, unused or verified: an undetected reuse remains possible, and the wording must not imply otherwise.
